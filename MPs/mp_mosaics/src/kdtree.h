...
class KDTree
{
  ...
  private:
    ...
    Point<Dim> findNearestNeighborHelper(const Point<Dim>& query, int dimension, KDTreeNode* subroot) const;
    void copy(KDTreeNode * subroot, KDTreeNode * other);
    Point<Dim>& select(vector<Point<Dim>> & newPoints, unsigned left, unsigned right, unsigned k, int depth);
    KDTreeNode * insert_kdtree_helper(vector<Point<Dim>>& newPoints, unsigned left, unsigned right, int depth);
    int partition(vector<Point<Dim>> & Points, unsigned left, unsigned right, unsigned pivotIndex, int depth);
