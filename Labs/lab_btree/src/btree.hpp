...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     * are looking for, we are done. */
     ...
     if (first_larger_idx < subroot->elements.size() && subroot->elements.at(first_larger_idx).key == key) {
        return subroot->elements.at(first_larger_idx).value;
     }
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    // if (!child->is_leaf) {
    //     new_right->children.assign(child->children.begin(), mid_child_itr);
    // }
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if (first_larger_idx < subroot->elements.size() && subroot->elements.at(first_larger_idx).key == pair.key) {
        return;
        ...
        if (subroot->children.at(first_larger_idx)->elements.size() >= order) {
