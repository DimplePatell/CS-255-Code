...
size_t insertion_idx_helper(const std::vector<T>& elements, const C& val, size_t start, size_t end)
{
    ...
    if (mid_index == elements.size() - 1 && val > elements.at(mid_index)) {
        return elements.size();
    }
    if (val > elements.at(mid_index) && val < elements.at(mid_index+1)) {
        return mid_index + 1;
    }
    if (val == elements.at(mid_index)) {
        return mid_index;
    }
    if (val < elements.at(mid_index)) {
        return insertion_idx_helper(elements, val, start, mid_index);
    } else {
        return insertion_idx_helper(elements, val, mid_index+1, end);
    }
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if (elements.empty() || elements.at(0) > val || elements.at(0) == val) { return 0; } 
