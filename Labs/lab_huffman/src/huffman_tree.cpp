...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while((singleQueue.size() + mergeQueue.size()) >= 2) {
        TreeNode* smallest_entry1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode* smallest_entry2 = removeSmallest(singleQueue, mergeQueue);
        if (smallest_entry1 != NULL && smallest_entry2 != NULL) {
            int newNodeFreq = smallest_entry1->freq.getFrequency() + smallest_entry2->freq.getFrequency();
            TreeNode* newNode = new TreeNode(newNodeFreq);
            newNode->left = smallest_entry1;
            newNode->right = smallest_entry2;
    ...
    while (bfile.hasBits()) {
         ...
         */
         ...
         if (current->left == NULL && current->right == NULL) {
            char temp = current->freq.getCharacter();
            ss << temp;
            current = root_;
        }
    }
