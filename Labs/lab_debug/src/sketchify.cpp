...
HSLAPixel* myFavoriteColor() {
    HSLAPixel *p = new HSLAPixel(280, 0.8, 0.5); //HSLAPixel p(164, 0.8, 0.5);
    return p; // return &p;
}
...
void sketchify(std::string inputFile, std::string outputFile) {
    ...
    for (unsigned y = 1; y < height; y++) { 
        for (unsigned x = 1; x < width; x++) {
            ...
            HSLAPixel *currOutPixel = &(output->getPixel(x, y)); //HSLAPixel currOutPixel = (*output).getPixel(x, y); original line
            if (diff > 20) {
                *currOutPixel = *myPixel;// currOutPixel
